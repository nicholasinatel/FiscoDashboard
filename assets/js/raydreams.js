...
(function ($) {
	...
	function loadData(data, keyField) {
		...
		for (var row = startRow - 1; row < ((base.currentPageIdx + 1) * base.pageSize) && row < data.length; ++row) {
			...
			for (; col < base.headers.length; ++col) {
				...
				// start the td
				if (data[row].description == "RECEITA BRUTA" ||
					data[row].description == "(-) DEDUÇÕES DA RECEITA" ||
					data[row].description == "(-) CUSTO DAS VENDAS E SERVIÇOS" ||
					data[row].description == "(-) DESPESAS OPERACIONAIS") {
					// console.log("data[row] being taken: " + JSON.stringify(data[row]));
					var cell = jQuery('<td class="special-markup"></td>');
				} else {
					var cell = jQuery('<td class="normal-markup"></td>');
				}
				...
				if (fieldName != null) {
					...
					if (base.headers[col].format != null) {
						...
						} else if (data[row][fieldName].indexOf("R$") != -1) {
							// console.log("row e fieldname: "+ row + fieldName);
							cell.remove();
							cell.addClass("real-markup");
							cell.append(accounting.formatMoney(data[row][fieldName], "R$", 0, ".", ","));
							list.push(accounting.formatMoney(data[row][fieldName], "R$", 0, ".", ","));
							if (fieldName == "value_date_start") {
								cell.addClass("lancamento");
								cell.append(' <button type="button" class="launch-btn-1">+</button>');
								// cell.attr({
								// 	'type': 'button'
								// });
							} else if (fieldName == "value_date_end") {
								cell.addClass("lancamento");
								cell.append(' <button type="button" class="launch-btn-2">+</button>');
							}
						} else if (data[row][fieldName].indexOf("%") != -1) {
							cell.remove();
							cell.addClass("percent-markup")
							cell.append(accounting.formatNumber(data[row][fieldName], "1", ".") + "%");
						}
	...
	function renderTable() {
		...
		var skel = jQuery('<table class="table table-bordered" style="margin-bottom:0px;"><thead><tr id="table-header-1"></tr></thead><tbody></tbody></table>');
		// add each header
	...
	function renderFooter(params) {
		...
		base.parentElem.find('#raytable-footer').remove();
